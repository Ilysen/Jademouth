...
namespace XRL.World.QuestManagers
{
	...
	public class ChaosTheoryManager : QuestManager
	{
		public override void OnQuestComplete()
		{
			...
			GameObject gameObject = zone.FindObject("Jademouth_Bright");
			if (gameObject != null)
			{
			...
			foreach (GameObject go in zone.FindObjects("Jademouth_ChaosTheorySconceSpawner"))
				go.CurrentCell.AddObject("Techlight1", null, null, null, null);
			foreach (GameObject go in zone.FindObjects("Jademouth_BlackoutCurtains").ToList())
			{
				Cell c = go.CurrentCell;
				go.Destroy(null, true, true, null);
				c.AddObject("Door", null, null, null, null);
			}
			foreach (GameObject go in zone.FindObjects("Jademouth_LightSign").ToList())
				go.Destroy(null, true, true, null);
			HandleReward();
